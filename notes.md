...
In actuality, using props is the suboptimal way of passing data between the parent and the child.

For example, we can't pass any HTML using props.

What would be easier would be if we could just pass the HTML block between the component

<app-quote>
  <html block>
</app-quote>

So the best way to do it is with SLOTS

// SLOTS //

So in the child component we simply use the:

<slot></slot>

within the child component template
...
But where does the code that is passed in get compiled, and how does it get style?

// Compiling and Styling HTML in a SLOT //

Styling that is applied in the child component will also be applied to whatever HTML is passed through the slot.

What if what is being passed is actually dynamic?

So in the parent, what we are passing to the slot below is actually interpolated data?

Well it works. The dynamic data is passed to the child through the slot. Everything, except for styling is handled on the parent component.

// How do we use multiple slots //

Easy. You just give the slot a name in the parent component by add ing a slot="nameofslot" to that which you want to pass.

Then, in the child component all that needs to be done is a name is given to the two slots.

<slot name="title"></slot>
<slot name="content"></slot>

The other thing to note about slots is that you can set a default value by just hardcoding something betweem the slot tags:

<slot name="subtitle">This is where the subtitle will go</slot>

Until the parent actually passes some data to subtitle, it will print "This is where the subtitle will go"

// Dynamic Components //

Dynamic components are pretty simple as well. Instead of <slot></slot> we are using <component></component>.

We can make it dynamic by linking it to a data property using the :is="value" keyword.

I our case, we are importing three separated components. Then using buttons, we are clicking to select the component that is displayed.

Our App.vue template looks like this:
...
On @click we are setting the selectedComponent data property to one of the importen components. Then, we simply use the :is keyword to link it to that dynamically changing property:

:is="selectedComponent"

Buuut the default mode of the this dynamic component is that when you switch the component that is being showed, the component that is not being showed actually is reset completley.

There is a way to overcome this default behaviour.

We can use the <keep-alive></keep-alive> wrapper which will preserve the component.

But the coolest thing to do is use Life-cycle hooks.

// Dynamic Components and Life-cycle hooks //

There are two lifecycle hooks that come with <keep-alive></keep-alive> that can be called on the child component.
...
REMEMBER THAT LIFECYCLE HOOKS SIT IN THE SCRIPT AS JUST FUNCTIONS
